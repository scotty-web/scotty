...
matchRoute (Capture pat) req = go (T.split (== '/') pat) (compress $ T.split (== '/') $ path req) []
    where
        ...
        go [] r prs
            | T.null (mconcat r) = Just prs -- in case request has trailing slashes
            | otherwise = Nothing -- request string is longer than pattern
        go p [] prs
            | T.null (mconcat p) = Just prs -- in case pattern has trailing slashes
...
parseRequestBody bl s r =
    case Parse.getRequestBodyType r of
        ...
        Just rbt -> do
            ...
            let provider = modifyMVar mvar $ \case
                    [] -> return ([], B.empty)
                    (b : bs) -> return (bs, b)
...
regex pattern = Function $ \req ->
    fmap
        (zipWith (curry (T.pack . show *** T.pack)) [0 :: Int ..] . strip)
