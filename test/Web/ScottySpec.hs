...
-- Skip these imports on Windows, since Unix sockets are not available
    ...
    describe "scottySocket" $ do
        it "works with a unix socket" $ do
            let ok = "HTTP/1.1 200 OK"
                ...
                listenOn path =
                    bracketOnError (socket AF_UNIX Stream 0) close $ \sock -> do
                        setSocketOption sock ReuseAddr 1
                        bind sock (SockAddrUnix path)
                        listen sock maxListenQueue
                        return sock

            withServer (Scotty.get "/scotty" $ html "") . E.bracket (socket AF_UNIX Stream 0) close $
                \sock -> do
